...
async def get_architecture(
    ...
    """Get the architecture layers for a repository"""
    ...
    if not scan or scan.status != "completed" or not scan.data:
        raise HTTPException(status_code=404, detail="Architecture not found or scan not completed")
    
    return scan.data.get("architecture", {"layers": []})
